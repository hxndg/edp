...
graph TD
    subgraph 触发层
        ...
        STUCK[platform_stuck_sensor: execution_claim 心跳看护]
...
| 表                  | 主键                     | 作用                                     | 读写方                                                                             |
| ------------------ | ---------------------- | -------------------------------------- | ------------------------------------------------------------------------------ |
| `upload_session`   | `upload_id`            | 上传事务状态机，**业务生命周期 SoT**              | 网关写 ready；**引擎**读 status / claim 收尾改 done；kafka sensor 不查此表；T+1 schedule / stuck 仍用 |
| `platform_job`     | `job_id`               | **通用异步任务状态机**（`job_type` 区分类型，MVP 有 `training`），新任务类型不再各建一张表 | 网关写 ready；**引擎**解读 status；kafka sensor 不查；watchdog 卡死看护 |
| `execution_claim`  | `(scope, business_id)` | 当前执行租约：owner run + 心跳，结束即删                    | 引擎 acquire/heartbeat/release；stuck sensor 看护                                    |
...
| `alerts`           | `alert_id`             | 业务终失败、卡死等异常落表待人工处理                         | stuck sensor / 引擎写入；人工消费                                                        |

**通用状态机协议**（`common/jobs.py`）：`upload_session` 跑通之后，训练等新任务类型如果都各自克隆一张状态表 + 一套 stuck/retry sensor，代码会线性膨胀。做法是把"状态机"抽成一份**协议 + 按类型注册的定制点**：

- **协议**：业务态 `ready → running → done/failed`；`execution_claim` 防双写；Argo 管 task retry；watchdog 只把心跳超时的运行任务收敛为 failed。
...
| `heartbeat_at` | Argo 轮询和 Iceberg 阶段续租；超时可由 watchdog 收敛 |
...
- PG 只保留业务终态、最终错误摘要和 `last_dagster_run_id`；中间 task 细节查 Argo。
- `execution_claim` 是临时租约，结束即删。未来若 claim 成为热点，可等价迁到 Redis TTL，不改变业务表与 Argo 契约。
...
sequenceDiagram
    ...
    opt 兜底路径（Kafka 消息丢失 / run 崩溃）
        ...
        DG->>PG: stuck sensor 发现 claim 心跳超时，置 failed 并释放 claim
    end
```
...
**Sensor 叫醒 vs 引擎解读 SoT**：Kafka sensor **不查** `upload_session.status`；是否真跑由引擎 `status <> 'done'` + `execution_claim` 决定。
...
| sensor | `training_kafka_sensor` 消费 Kafka、背压、一个 job 一个 RunRequest，**不查 PG**；`platform_stuck_sensor` 只看护超时 claim |
...
- run 成功 claim 后写 `last_dagster_run_id`；Argo 负责单次 Workflow 内 task retry。
- run 最后一次性写 `platform_job.done/failed`、结果或最终错误，再释放 claim。
- watchdog 只把心跳超时的 running 任务置 failed；人工 retry 仅允许 failed → ready + Kafka。
...
2. ✅ `common/jobs.py`：`JobKind` 注册表 + `watchdog_pass` 通用看护 + `create_job`/`manual_retry` 协议原语；`orchestration/sensors.py` 的 stuck 逻辑迁移至此。
3. ✅ `engines/worker/train_mock.py` / `engines/training/run_job.py`：worker 与 run 侧控制面。
4. ✅ `orchestration/`：`model_training` asset + `model_training_job` + `training_kafka_sensor` + `platform_stuck_sensor` + `training_quality_gate`。
...
| 卡死收敛       | `platform_stuck_sensor` 找超时 claim，业务态置 failed 并释放租约 | 入湖/训练                 |
