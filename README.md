...
| 模型训练与管理（MLflow） | `POST /train` 发起真实（小规模）训练：worker pod 训练 + MLflow 逐 epoch 打点，终局档案归档 Iceberg，Registry promote 流转版本（3.7） | 科研人员                 |
...
1. **编排层面：**
  ...
  2. **触发收敛进编排器**：Kafka sensor 只负责叫醒、组批和背压；真正的互斥由引擎入口的 `execution_claim` 保证。
2. **执行层：**
...
3. **存储曾：**
  ...
  2. **幂等写入，安全重试**：所有作业按批次/分区做 `MERGE`/`overwrite`；Argo 管 task retry，Dagster 单写者最终提交 Iceberg 与业务终态。
...
graph TD
    subgraph 触发层
        ...
        KSENSOR[kafka sensor ×2: ingest.requested / jobs.requests]
        STUCK[reconciliation schedule: 核对 Dagster/Argo 后收敛状态]
    ...
    subgraph 执行层
        SPARK[Spark local mode<br/>Dataset freeze / 导出]
        DUCK[DuckDB<br/>规则打标签 / 分析查询资产]
        RAY[Ray local mode<br/>自动预标 mock / 数据质检]
        WORKER[worker pod fan-out · Argo Workflows 监管<br/>ingest 解析 / 模型训练]
        ML[MLflow<br/>实验记录 + Model Registry 操作台]
    end

    subgraph 存储层
        MINIO[(MinIO<br/>OSS 替身: raw/bronze/silver/gold/dataset/models/staging)]
        ICE[(Iceberg + REST Catalog<br/>结构化表 + 血缘 SoT)]
        LANCE[(Lance<br/>样本本体/embedding)]
        PG[(PostgreSQL: platform 库<br/>业务状态/execution_claim/Alerts)]
        KAFKA[(Kafka<br/>业务事件账本 + 触发 topic ×2)]
    end
...
| GPU/ML 执行 | Ray                                             | Ray local mode，`@ray.remote`                       | 自动预标（mock）、自动数据质检（真实规则，见 3.4）；模型训练走独立 worker pod（3.7），将来可平滑替换为 Ray Train |
| 实验记录 + 模型注册 | MLflow                                       | `mlflow server`（复用 edp 镜像；backend=PG `mlflow` 库，artifact=MinIO `s3://lake/mlflow`） | 训练过程观测（逐 epoch 曲线、实验对比 UI）+ Model Registry（版本/alias 流转的操作台）；**不是 SoT**，终局记录由归档 asset 誊进 Iceberg，见 3.7 |
| worker pod 监督 | Argo Workflows                              | namespace 级安装（v4.0.7），日志归档到 MinIO `s3://lake/argo/` | task 扇出、并发、retry、超时、phase/exit/message 与日志 |
...
| 业务状态存储    | PostgreSQL（`platform` 库）                        | 独立 schema，与 Dagster 元数据库（`dagster` 库）、MLflow backend（`mlflow` 库）分开 | UploadSession、PlatformJob、ExecutionClaim、Alerts、DatasetRequest |
...
全平台 ID 均为**确定性派生**：`upload_id → episode_id ("ep-{upload_id}") → sample_id ("{episode_id}-w{窗口序号}") → anno_id / qc_id`。同一批数据重跑仍生成相同 ID，这是幂等 upsert 与直接血缘反查的基础。
...
| `ml_training_run`        | `job_id`                          | 训练归档（`engines/training/run_job.py`） | 训练档案：完整复现配方 + 终局指标（3.7.3），**模型域记录的 SoT** |
| `ml_metric_epoch`        | `(job_id, metric_name, epoch)`    | 同上（best-effort 从 MLflow 抄）      | 逐 epoch 曲线的持久底账；MLflow 清理后仍可查            |
| `ml_model_version`       | `(model_name, version)`           | 同上                             | 模型版本登记：权重 URI + sha256，闭合"数据→模型"血缘链     |
| `ml_promotion_event`     | `event_id`（追加式账本）            | gateway promote API             | stage 流转事件：谁、何时、从哪到哪；可重放出当前指针        |
| `ml_eval_sample`         | `(job_id, sample_id)`             | 训练 worker 产出、run pod 落表        | 逐样本评测明细（预测 vs 标注），供 DuckDB 分析模型退步位置   |
...
`ml_training_run`（训练档案，一次训练一行，详见 3.7.3；替代早期 mock 的 `train_run`/`eval_run`/`model_artifact` 三张占位表）：

| 字段                  | 类型        | 说明                                             |
| ------------------- | --------- | ---------------------------------------------- |
| `job_id`            | STRING    | **主键**，即 `platform_job.job_id`，PG 状态机与 Iceberg 档案的关联键 |
| `model_name`        | STRING    | 训练的目标模型名（Registry 里的注册名）                       |
| `dataset_name` / `dataset_version` | STRING | 外键指向 `dataset`：训练消费了哪个冻结数据集，血缘的关键一跳 |
| `mlflow_run_id` / `mlflow_experiment` | STRING | 到 MLflow UI 的跳转键；MLflow 不可用时为空，档案本身不缺任何决策字段 |
| `image`             | STRING    | 训练执行镜像（代码版本）                                   |
| `params_json`       | STRING    | 全部超参，JSON 编码                                   |
| `seed`              | LONG      | 随机种子——`dataset_version + image + params + seed` 四元组构成完整复现配方 |
| `metrics_json`      | STRING    | 终局指标（train_loss / val_loss / val_accuracy / 样本量），JSON 编码 |
| `artifact_uri` / `artifact_sha256` | STRING | 权重文件位置（MinIO）与指纹                        |
| `status`            | STRING    | `SUCCEEDED` / `FAILED`（终态才归档）                  |
| `trained_at`        | TIMESTAMP | 训练完成时间                                         |

`ml_metric_epoch`（逐 epoch 指标曲线，run pod 归档时从 MLflow 抄一份到 Iceberg）：

| 字段            | 类型        | 说明                                  |
| ------------- | --------- | ----------------------------------- |
| `job_id`      | STRING    | **复合主键之一**，关联 `ml_training_run`     |
| `metric_name` | STRING    | **复合主键之一**，如 `train_loss` / `val_loss` |
| `epoch`       | LONG      | **复合主键之一**                          |
| `value`       | DOUBLE    | 指标值                                 |
| `mlflow_run_id` | STRING  | 抄录来源                                |

`ml_model_version`（模型版本登记）：

| 字段                | 类型        | 说明                                        |
| ----------------- | --------- | ----------------------------------------- |
| `model_name`      | STRING    | **复合主键之一**                                |
| `version`         | STRING    | **复合主键之一**，即 `job_id`（每次成功训练产出一个不可变版本）    |
| `mlflow_version`  | STRING    | MLflow Registry 里对应的版本号（操作台侧的键；MLflow 不可用时为空） |
| `job_id`          | STRING    | 外键，产出该版本的训练任务                             |
| `dataset_version` | STRING    | 外键，训练所用数据集版本                              |
| `artifact_uri` / `artifact_sha256` | STRING | 权重位置与指纹（消费方校验用）             |
| `registered_at`   | TIMESTAMP | 登记时间                                      |

`ml_promotion_event`（stage 流转账本，只追加不修改）：

| 字段                       | 类型        | 说明                                 |
| ------------------------ | --------- | ---------------------------------- |
| `event_id`               | STRING    | **主键**                             |
| `model_name` / `version` | STRING    | 流转对象                               |
| `from_stage` / `to_stage`| STRING    | 如 `none → staging → production`    |
| `actor` / `reason`       | STRING    | 谁、为什么                              |
| `occurred_at`            | TIMESTAMP | 发生时间；按时间重放即可得到任意时刻的"当前 production 指针" |

`ml_eval_sample`（逐样本评测明细）：
...
| `y_true` / `y_pred`    | LONG    | 标注 vs 预测                    |
| `prob`                 | DOUBLE  | 预测置信度                       |
| `correct`              | BOOLEAN | 是否命中，DuckDB 聚合"模型在哪类数据上退步"用 |
...
DDL 定义在 `schemas/postgres_platform.sql`。PG 是任务业务生命周期的 SoT，承载高频状态更新、CAS/fencing 与毫秒级点查；Iceberg 是已提交数据事实的 SoT。两者职责不同。
新环境初始化会直接执行该文件；已有环境升级先执行
`psql "$DATABASE_URL" -f schemas/migrate_execution_claim.sql`，它会幂等新增业务
attempt 字段、claim 表和 manifest 冻结 trigger，再滚动发布应用。

##### 3.1.2.1 业务状态表

| 表                  | 主键                     | 作用                                     | 读写方                                                                             |
| ------------------ | ---------------------- | -------------------------------------- | ------------------------------------------------------------------------------ |
| `upload_session`   | `upload_id`            | 上传事务状态机，**业务生命周期 SoT**              | 网关写 ready；**引擎**只处理 ready；T+1 schedule / reconciliation 查询 |
| `platform_job`     | `job_id`               | **通用异步任务状态机**（`job_type` 区分类型，MVP 有 `training`），新任务类型不再各建一张表 | 网关写 ready；**引擎**只处理 ready；reconciliation 对账 |
| `execution_claim`  | `(scope, business_id)` | 当前执行租约：owner run + 心跳，结束即删                    | 引擎 acquire/heartbeat/release；reconciliation 看护 |
| `processing_type_definition` | `processing_type` | 业务处理类型到策略和当前执行 Profile 的可切换绑定 | 网关校验；run 启动时解析并冻结 |
| `worker_execution_profile` | `profile_id` | 不可变 Argo 模板、镜像和资源配置 | run 只读；任何变更插入新行 |
...
| `alerts`           | `alert_id`             | 业务终失败、执行消失等异常落表待人工处理                         | reconciliation job / 引擎写入；人工消费 |

**通用状态机协议**（`common/jobs.py`）：`upload_session` 跑通之后，训练等新任务类型如果都各自克隆一张状态表 + 一套 stuck/retry sensor，代码会线性膨胀。做法是把"状态机"抽成一份**协议 + 按类型注册的定制点**：

- **协议**：业务态 `ready → running → done/failed`；只有 `ready` 可启动；`execution_claim` 防双写；Argo 管 task retry；reconciliation 在 Dagster/Argo 均不活跃后才收敛为 failed。
- **定制点**：状态表/列、claim scope 和 Kafka 触发消息由每个 `JobKind` 声明。
- 新增一类任务 = 注册一条 `JobKind` + 写它的 run/worker 引擎，**不新增表、不新增看护 sensor**。

`platform_job`（通用任务状态机的默认载体）：

| 字段           | 类型          | 说明                                                   |
| ------------ | ----------- | ---------------------------------------------------- |
| `job_id`     | TEXT        | **主键**，网关生成（如 `train-xxxx`）                          |
| `job_type`   | TEXT        | 任务类型，MVP 只有 `training`；(job_type, status) 建索引        |
| `status`     | TEXT        | `ready → running → done / failed`（协议状态集）             |
| `payload`    | JSONB       | 类型专属输入（training：model_name / dataset_version / params / seed），协议层不解释 |
| `result`     | JSONB       | 终态摘要（training：终局指标 + 版本号 + MLflow 跳转键），供网关点查，深查走 Iceberg |
| `requested_by` | TEXT      | 请求人                                                  |
| `last_dagster_run_id` | TEXT | 最近一次真正 claim 并处理该任务的 Dagster run，便于快速跳转 |
| `last_execution_profile_id` | TEXT | 最近一次 run 实际冻结并使用的执行 Profile |
| `last_error_code` / `last_error` | TEXT | 最终业务失败摘要；task 细节仍查 Argo |
| `execution_attempt_count` | INT | 真正从 ready 启动的业务执行次数；Argo task 内部 retry 不计入 |
| `created_at` / `updated_at` | TIMESTAMPTZ | 审计时间戳；`updated_at` 兼作悬置判定与 run_key 组成 |

`upload_session` 有三个正交的业务字段：`manifest_op` 表达怎么改湖表，`pipeline_profile` 表达下游是否需要人工，`processing_type` 表达这份上传采用哪类处理逻辑。
...
| `processing_type`  | TEXT        | 业务处理类型，如 `mcap_imu`；Kafka 和 sensor 按它拆成不同 run |
| `status`           | TEXT        | `created → uploading → ready → ingesting → done / failed`              |
| `manifest_uri` / `manifest` | TEXT / JSONB | manifest 存放位置及内容快照；进入 ready 后冻结，修正须新建 upload |
| `last_dagster_run_id` | TEXT | 最近一次真正 claim 并处理该 upload 的 Dagster run |
| `last_execution_profile_id` | TEXT | 最近一次 run 实际使用的不可变执行 Profile |
| `last_error_code` / `last_error` | TEXT | 最终业务失败摘要；task phase/exit/retry/log 查 Argo |
| `execution_attempt_count` | INT | 真正从 ready 启动的业务执行次数；旧消息被跳过不计入 |
| `created_at` / `updated_at` | TIMESTAMPTZ | 审计时间戳；`updated_at` 兼作悬置判定依据                                    |

`execution_claim`（临时执行租约，详见 3.3）：

| 字段 | 说明 |
| --- | --- |
| `scope` + `business_id` | 复合主键，锁粒度始终是单个 upload/job |
| `run_id` | 当前 owner，也是 fencing token |
| `heartbeat_at` | Argo 轮询和 Iceberg 阶段续租；超时后由 reconciliation 核对 Dagster/Argo |
...
`runtime_config` 只放微批大小、背压、保留天数和质量门等平台级动态参数。Argo 模板、镜像、超时、并发、内存档和 chunk 大小属于可复现的执行配置，统一放入不可变 Profile。

##### 3.1.2.3 业务处理类型与不可变执行 Profile

`processing_type_definition` 是可修改的业务路由指针：

```text
mcap_imu → worker module + silver strategy → ingest-mcap-v1
training_mock → training worker           → training-mock-v1
```

`worker_execution_profile` 是只增不改的执行快照，包含 `workflow_template_name`、`contract_version`、镜像引用、三档内存、超时、并行度和 chunk 大小。数据库 trigger 禁止 UPDATE/DELETE；模板或镜像变化时创建新的 `profile_id`，再原子切换 definition 的 active 指针。生产镜像必须使用 digest。

run 以 `processing_type` 查询 definition 与 Profile，一次 JOIN 后在内存中冻结到 run 结束；成功 claim 时把 `profile_id` 写入 `last_execution_profile_id`。已经运行的任务不受后续指针切换影响，Workflow annotation 同时记录 type、profile 和 image。

已有部署先执行 `schemas/migrate_processing_profiles.sql`；新部署由 `postgres_platform.sql` 直接创建并播种默认 `mcap_imu` / `training_mock`。

上线新版本时只做“新增 Profile + 切指针”，不改旧行：
...
INSERT INTO worker_execution_profile (
  ...
  'ingest-mcap-v2', 'edp-worker-batch-v2', 'v1',
  'registry.example/edp@sha256:...', '["2Gi","4Gi","8Gi"]', 900, 30, 50000
);
UPDATE processing_type_definition
SET active_execution_profile_id = 'ingest-mcap-v2', updated_at = now()
WHERE processing_type = 'mcap_imu';
```

##### 3.1.2.4 触发、执行与状态分工

```text
Kafka → Sensor（按 manifest_op + processing_type 组批，不查 PG）
      → Dagster run（从 PG 冻结 Profile + acquire execution_claim）
      → Argo（task 并发/retry/phase/exit/log）
      → Dagster（Iceberg 单写者 + PG 最终汇总 + release claim）
```

- Kafka 消息可以重复；是否真做由引擎入口的业务状态和 claim 决定。
- PG 保留业务终态、业务执行次数、最终错误摘要和 `last_dagster_run_id`；中间 task retry 细节查 Argo。
- `execution_claim` 是临时租约，结束即删。心跳超时只产生对账候选；定时 reconciliation run 查询 Dagster/Argo，确认两者均不活跃才释放 claim、置 failed 并写 alert，绝不自动发 Kafka。
- `ready` 是唯一可执行状态；manifest 一旦进入 ready 即冻结。用户修正内容须创建新 upload，失败重跑须走 retry API 显式执行 `failed → ready`。
...
- **正向（模型 → 原始文件）**：`ml_model_version.dataset_version → dataset_sample.sample_id → sample.episode_id → episode_file.file_uri → raw_file`，每一跳都是主外键 join，不依赖任何运行时系统。
...
#### 3.2.1 上传入湖（按操作语义和业务处理类型拆 run）

Kafka 消息携带 `upload_id + manifest_op + processing_type`。Sensor 不查业务表，按 `(manifest_op, processing_type)` 组批：操作语义不同进入 append/correct job，业务处理类型不同也一定进入不同 Dagster run。每个 run 再从 PG 解析当前执行 Profile，并只提交一个对应模板和镜像的 Argo Workflow。
...
sequenceDiagram
    ...
    C->>GW: 创建 session（声明 manifest_op / pipeline_profile / processing_type）
    GW-->>C: upload_id + 预签名 URL
    C->>MINIO: 直传 MCAP 文件
    C->>GW: 提交 manifest
    GW->>PG: upload_session.status = ready
    Note over C,PG: ready 后 manifest 冻结；修正内容创建新 upload
    GW->>K: 发 ingest.requested（edp.ingest.requests）

    K-->>DG: ingest_kafka_sensor 消费（offset 存 sensor cursor，微批合并见 3.6.2）
    DG->>RUN: 每 (op,type) 一个 RunRequest（run_config 传 upload_ids + processing_type）

    RUN->>PG: 解析 definition/Profile；execution_claim 逐 upload CAS 抢占
    RUN->>ARGO: 一个 Workflow，批内每 upload 一个 task
    ARGO-->>RUN: manifest + 最终 task outcome
    RUN->>ICE: INDEX→BRONZE→SILVER→SAMPLES（每表每批一次 commit）
    Note over RUN,ICE: correct 只 replace_where 覆盖受影响分区，<br/>并把受影响 sample 的 annotation/qc_result 置 pending
    RUN->>PG: 最终事务：upload_session done/failed + release claim

    opt 兜底路径（Kafka 消息丢失 / run 崩溃）
        DG->>PG: T+1 schedule 轮询 status=ready 补触发
        DG->>PG: reconciliation run 核对 Dagster/Argo；均不活跃才置 failed + alert
    end
```



可靠性依赖三层：Kafka offset 与 RunRequest 一起持久化；run_key 抑制相同批次重复提交；最终正确性由引擎读取 PG 业务态和 `execution_claim` CAS 保证。重放消息可能产生空 run，但不会产生双写。
...
#### 3.2.4 模型训练与归档（MLflow，详细设计见 3.7）
...
sequenceDiagram
    ...
    participant PG as Postgres
    participant DG as Dagster run pod
    participant W as 训练 worker pod
    participant ML as MLflow
    participant ICE as Iceberg

    rect rgb(255, 243, 205)
        Note over U,GW: 【人工参与】提交训练请求
        U->>GW: POST /train（model_name / dataset_version / params）
    end
    GW->>PG: platform_job 落 ready（job_type=training）+ 发 Kafka 触发
    Note over DG: training_kafka_sensor 消费，一个 job 一个 run
    DG->>PG: execution_claim（scope=training）→ status=running + last_run_id
    DG->>W: 解析 dataset_sample 清单写进 staging，提交 Argo Workflow 拉起 worker
    W->>W: 读 Lance 样本，SGD 逐 epoch 训练
    W->>ML: log_params + 逐 epoch log_metrics（best-effort，挂了照常训练）
    W->>W: 权重传 MinIO models/，评测明细写 staging，manifest.json 收尾
    DG->>ICE: 归档：ml_training_run / ml_metric_epoch / ml_model_version / ml_eval_sample
    DG->>PG: 最终事务：status=done + result + release claim
    rect rgb(255, 243, 205)
        Note over U,ML: 【人工参与】看曲线/对比实验走 MLflow UI；promote 走 gateway API
    end
```
...
graph TD
    ...
    DATASET -->|"worker pod + MLflow"| TRAIN["model_training<br/>(ml_training_run / ml_model_version / ...)"]
...
- **PostgreSQL 业务态**：`upload_session` / `platform_job` 保存 `status`、最终错误摘要和 `last_dagster_run_id`，方便直接跳转最近一次处理记录。
- **薄执行租约**：`execution_claim(scope, business_id, run_id, heartbeat_at)` 只做互斥、心跳和 fencing；结束即删除，不复制 task 终态。
- **Argo 执行态**：一个批次一个 Workflow；每个 upload 一个 task。Argo 管并发、retry、OOM 内存升档、phase/exit/message 和日志。
- **Dagster 最终汇总**：等待 Workflow 终态，读取成功产物，由单写者批量提交 Iceberg，最后在一个事务里更新业务终态并释放 claim。

Iceberg 各表写入仍采用 upsert / `replace_where` 等幂等操作。中途崩溃后可从头重跑；下游通过 `common/ingest_state.py:uncommitted_episode_ids()` 排除尚未 `done` 的 episode，避免读取半成品。
...
> **实施状态**：已实施。触发/背压在 `orchestration/sensors.py`，薄租约在 `common/execution_claim.py`，批处理引擎在 `engines/spark/`，Argo 构造在 `common/argo_workflows.py`。
...
tick
 ...
 ├─ 4. 按 (manifest_op, processing_type) 分组（**不查 PG**）
 └─ 5. 每组 = 一个批次 = 一个 RunRequest = 一个 run pod：
        run_config 携带 upload_ids + processing_type，
        run_key/tags 同时带 op 与业务类型
```

**Sensor 叫醒 vs 引擎解读 SoT**：Kafka sensor **不查** `upload_session.status`；是否真跑由引擎 `status = 'ready'` + `execution_claim` 决定。旧消息带来的 run 会廉价跳过，不会重启 ingesting/failed。

**批的边界 = (tick, manifest_op, processing_type)**。同一 tick 内不同业务处理逻辑进入不同 Dagster run，因而可以各自冻结不同模板、镜像和资源 Profile；每个 run 仍只提交一个 Argo Workflow。
...
#### 3.6.3 批内执行：Argo task，Dagster 单写者

claim 粒度是稳定的 `upload_id`，而不是每次都会重组的批次。一个批次只提交一个 Workflow CR；`spec.parallelism` 控制同时运行的 worker 数。

```text
run pod（控制面 + 单写者）                     Argo Workflow（一个批次一个 CR）
──────────────────────────────────           ─────────────────────────────
PG 解析 processing_type → 不可变 Profile
批量 acquire claim；写 last_run_id + last_execution_profile_id
写 input.json 到 staging
提交 Workflow CR（名字确定性生成，重复提交幂等）─▶  每 upload 一个节点并行起 pod：
轮询 workflow status（每 tick 续 claim）            下载 MCAP → 流式解析/清洗 → 写 Lance
收 manifest.json + 最终 Argo outcome ◀──────────    Argo 按结构化退出码重试，失败不取消同批其它 task
每表每批一次 Iceberg commit（事务内分块追加）
一个 PG 事务批量写 done/failed + 最终错误摘要 + release claim
```

- Profile 从 PG 指定版本化 WorkflowTemplate（如 `edp-worker-batch-v1`）、镜像和资源；模板与 Profile 都不原地修改。
- 确定性数据错误退出 10，不重试；存储抖动、未知崩溃、OOM/137 等由 Argo 重试，最多三档内存。
- `failFast: false`，坏 upload 不取消好 upload。最终 outcome 带 phase、exit code、message、retry count 和全部 pod/log URI。
- worker 不连 PG、不碰 Iceberg catalog；厚表走 staging parquet，薄表行放 manifest。Iceberg commit 始终收敛在 Dagster run 单写者。
...
3. ✅ `common/execution_claim.py`：批量 acquire/heartbeat/check/release，全部以 run_id fencing。
...
5. ✅ pod fan-out：一个批次一个 Workflow CR，稳定 WorkerTemplate + Argo retry/内存升档/日志归档。
6. ✅ 最终状态：PG 业务表保存最后 run 与最终错误；task 详情不复制，查 Argo。
7. ✅ retention：清终态业务行、孤儿 execution_claim 和 staging 残留。
8. ⏳ backlog：`entity_tag_index` 从全量重建改增量（按本批 target 同步），保留全量重建入口做对账（3.5.1 相应更新）。当前数据量全量重建耗时可接受，暂不动。

### 3.7 模型训练与管理（MLflow 集成）

模型也是 asset：它由确定的数据版本、代码与超参产出，进入同一张血缘图，并复用 claim + Argo retry + 业务终态协议。

#### 3.7.1 MLflow 的角色边界：操作台，不是 SoT

MLflow 是**实验记录与模型注册的操作台**——logging SDK（训练进程里 `log_params`/`log_metrics`）、Tracking Server（实验对比 UI、逐 epoch 曲线）、Model Registry（模型版本与 alias 流转）。这三样自建极易出错（指标断点续传、并发注册、UI），所以引入现成组件而不是手搓。

但 MLflow 在本平台的定位与 Iceberg 有明确分界，与 OpenSearch 之于 tag 检索（3.5）完全同构：

| 维度       | MLflow                                | Iceberg（SoT）                                    |
| -------- | ------------------------------------- | ----------------------------------------------- |
| 回答的问题    | "训练进行得怎么样？哪次实验更好？"（过程，人看）             | "这个模型版本是用哪些数据、什么配方训出来的？"（终局，审计/血缘/程序消费）          |
| 写入方      | 训练 worker **best-effort** 打点，挂了不影响训练   | run pod 单写者在终态时归档                                  |
| 丢了会怎样    | 丢过程观测，可接受；终局记录能从 Iceberg 反向补登          | 不允许丢——所以归档失败整个 job 算失败，会按码重试                     |
| 部署形态     | `mlflow server`：backend=PG `mlflow` 库，artifact 经 server 代理写 MinIO `s3://lake/mlflow` | 既有 REST catalog + MinIO 仓库 |

**一致性模型刻意不做双写事务**：worker 打点 MLflow 与 run pod 归档 Iceberg 是两条独立链路，前者尽力而为，后者失败则整个业务任务失败。模型权重 SoT 在 MinIO `models/`。

#### 3.7.2 数据流与训练执行形态

训练复用 3.6.3 的 pod fan-out 骨架（Argo Workflow 监管、staging 契约、错误码），只是 fan-out 数为 1（一次训练一个 worker）：

```
gateway POST /train                          训练 worker pod（每 job 一个）
  └ platform_job(ready) + Kafka 触发
training_kafka_sensor（背压 TRAIN_MAX_INFLIGHT）
  └ 一个 job = 一个 RunRequest
run pod：execution_claim（scope=training）
解析 dataset_sample 清单 → samples.parquet
写 input.json 到 staging ────────────────▶  读清单，逐样本从 Lance 抽特征
                                            SGDClassifier 逐 epoch 训练
                                            ├ MLflow log_metrics（best-effort）
                                            ├ 权重直传 MinIO models/（算 sha256）
                                            ├ 评测明细写 staging eval.parquet
                                            └ manifest.json 收尾（指标/版本/error_code）
收 manifest ◀────────────────────────────
归档四表（每表一次 commit，job_id 幂等重写）
最终事务 → platform_job done + result + release claim
```

- **数据入口有两种圈选方式，殊途同归到 `dataset_version`**：批量条件圈选走既有 freeze（tag 过滤 + 质量门）；人工挑样本走 `POST /dataset-requests` 的 `sample_ids` 字段——跳过 Spark 条件计算，直接校验样本存在后冻结成一个新的 dataset_version。**训练永远只认 dataset_version**，不接受裸 sample 列表，保证任何一次训练都可复现、可追溯。
- **worker 依旧无状态**：只碰 staging / Lance / MinIO models/ / MLflow HTTP，不连 PG、不碰 Iceberg catalog。样本清单（sample_id / lance_uri / split / 标签）由 run pod 预先物化成 `samples.parquet` 放进 staging。
- **训练是真训练但刻意小**：从 Lance 样本抽 IMU 统计特征，`SGDClassifier` 逐 epoch `partial_fit` 学"质量分档"二分类——足够产生真实的 loss 曲线、可评测的预测，又不需要 GPU。将来换 Ray Train/真模型只改 worker 内部，契约（input.json / manifest.json / MLflow 打点）不变。
- **复现配方四元组**：`dataset_version + image + params_json + seed` 全部落 `ml_training_run`。seed 不填由 gateway 生成后固定，同配方重跑逐 epoch 曲线可比。
- **归档 = run pod 单写者的一次前向恢复**：`ml_training_run`（upsert by job_id）、`ml_metric_epoch` / `ml_eval_sample`（`replace_where` by job_id）、`ml_model_version`（upsert by (model_name, version)），全部以 job_id 为幂等键——重试/接管后重写同一批行，不产生重复档案。
- **promote 走 gateway**：`POST /models/{name}/versions/{version}/promote` 同步做两件事：调 MLflow Registry 设 alias（操作台指针）+ 向 `ml_promotion_event` 追加一条事件（审计账本）。事件账本是追加式的，两写之间崩了顶多"alias 已改、账本没记"，重放 API 幂等补齐。

#### 3.7.3 Dagster 改动

| 对象 | 改动 |
| --- | --- |
| asset | 新增 `model_training`（group=training，dep 指向 `dataset`——血缘图上"数据→模型"的那条边）；删除 mock 的 `mock_train_assets` |
| job | 新增 `model_training_job`；删除 `mock_train_job` |
| sensor / schedule | `training_kafka_sensor` 消费 Kafka、背压、一个 job 一个 RunRequest，**不查 PG**；`reconciliation_schedule` 定时启动独立对账 run |
| asset check | 新增 `training_quality_gate`：val_accuracy ≥ `TRAIN_GATE_MIN_ACCURACY` 才算过，UI 上给出独立的质量结论卡片（不挡归档——训得差也是要记录的事实，挡的是 promote 的手） |
| 引擎 | `engines/training/run_job.py`（run 侧控制面：claim→物化清单→fan-out→收 manifest→归档四表）；`engines/worker/train_mock.py`（worker 入口）；删除 `engines/ray/mock_train.py` |

失败语义复用 `common/errors.py`：`DATA_EMPTY` 等确定性业务错误不重试；OOM/基础设施错误由 Argo 在 Workflow 内重试，并按该 training Profile 的内存档升档。MLflow 不可达仍是 best-effort。

#### 3.7.4 与 ingest 共用的运行协议

- `common/jobs.py` 的 `JobKind` 只描述业务表、状态名、claim scope 与触发消息。
- run 成功 claim 且仍为 ready 后递增 `execution_attempt_count`、写 `last_dagster_run_id`；Argo 负责单次 Workflow 内 task retry。
- run 最后一次性写 `platform_job.done/failed`、结果或最终错误，再释放 claim。
- reconciliation 仅在 Dagster/Argo 都不活跃时把 running 任务置 failed 并告警，不自动重试；人工 retry 仅允许 failed → ready + Kafka。
- 归档以 `job_id` 为幂等键，重复执行覆盖同一份档案。

#### 3.7.5 使用方式（科研人员视角）

```bash
# 1. 冻结数据（二选一）：条件圈选 / 手工挑样本
curl -X POST gateway:8000/dataset-requests -d '{"dataset_name":"grasp-v1","filter_expr":{"quality_band":"high"}}'
curl -X POST gateway:8000/dataset-requests -d '{"dataset_name":"grasp-picked","sample_ids":["sample-a","sample-b"]}'

# 2. 发起训练（返回 job_id）
curl -X POST gateway:8000/train -d '{"model_name":"grasp-cls","dataset_version":"v20260717...","params":{"epochs":8}}'

# 3. 看进度/结果：点查走 gateway，过程曲线走 MLflow UI（port-forward svc/mlflow 5000）
curl gateway:8000/training-jobs/{job_id}          # PG platform_job：status + result 摘要
#    深度分析走 DuckDB 查 ml_training_run / ml_eval_sample / ml_metric_epoch

# 4. 满意后 promote（写 MLflow alias + ml_promotion_event 账本）
curl -X POST gateway:8000/models/grasp-cls/versions/{job_id}/promote -d '{"stage":"production","actor":"alice"}'
```

#### 3.7.6 改动清单（实施状态）

1. ✅ `schemas/postgres_platform.sql` + `schemas/iceberg_tables.py`：`platform_job` DDL、五张 ML 表、`TRAIN_*` runtime_config 默认值；删 `train_run`/`eval_run`/`model_artifact` 占位定义。
2. ✅ `common/jobs.py`：`JobKind` 注册表 + `reconciliation_pass` 通用对账 + `create_job`/`manual_retry` 协议原语。
3. ✅ `engines/worker/train_mock.py` / `engines/training/run_job.py`：worker 与 run 侧控制面。
4. ✅ `orchestration/`：`model_training` asset + `model_training_job` + `training_kafka_sensor` + `reconciliation_job/schedule` + `training_quality_gate`。
5. ✅ `gateway/`：`POST /train`、`GET /training-jobs/{id}`、`POST /training-jobs/{id}/retry`、`GET /models/{name}/versions`、promote API；`/dataset-requests` 支持 `sample_ids`。
6. ✅ 部署：`10-infra.yaml` 加 mlflow Deployment/Service（PG `mlflow` 库 initContainer 幂等建库；backend 必须走 `postgresql+psycopg2` 方言——psycopg3 下 MLflow 的查询会报 "integer = character varying" 类型错误）；`edp-env` 加 `MLFLOW_TRACKING_URI` / `KAFKA_JOBS_TOPIC`；pyproject 加 `mlflow`、`scikit-learn`、`psycopg2-binary`。**附带修复**：iceberg-rest 的 catalog 元数据原先落进程内 SQLite，pod 一重启全部表注册信息丢失（数据文件还在 MinIO 但 catalog 不认识）——已改为 JDBC 落 PG `iceberg_catalog` 库（initContainer 幂等建库）。
7. ✅ 训练成功/失败、人工 retry 与 promote 链路已覆盖；task retry 与 attempt 详情以 Argo 为准。
...
| 触发需求       | Dagster 机制                                                                                  | 用在哪               |
| ---------- | ------------------------------------------------------------------------------------------- | ----------------- |
| 近实时事件触发    | `ingest_kafka_sensor` 消费 `edp.ingest.requests`（offset 存 sensor cursor，与 RunRequest 提交原子持久化；微批合并 + 背压见 3.6.2）；`training_kafka_sensor` 消费 `edp.jobs.requests`（3.7） | 上传入湖（见 3.2.1）、模型训练（3.7） |
| 定时 T+1 兜底  | `ScheduleDefinition` 轮询 PG `status=ready`，互斥靠 execution_claim | 入湖（防 Kafka 消息丢失）  |
| 卡死收敛       | `reconciliation_schedule` 每 5 分钟建 run；核对 Dagster/Argo 均不活跃后置 failed、释放租约并告警，不自动重试 | 入湖/训练 |
...
| 活                                    | 特征                 | 引擎                           |
| ------------------------------------ | ------------------ | ---------------------------- |
| Bronze/Silver 解析（ingest）             | 每 upload 独立、失败要隔离  | worker pod fan-out，Argo Workflows 监管（3.6.3），run pod 单写者 commit |
| Dataset freeze、导出                    | 结构化、SQL 味、批量       | Spark local mode             |
| 规则打标签、分析类查询资产                        | 小批量（本地几 GB 内）、秒起秒停 | DuckDB（直接 `iceberg_scan`）    |
| 自动预标（mock）、数据质检（3.4 真实规则）             | Python + 模型形态      | Ray local mode，`@ray.remote` |
| 模型训练（3.7）                            | 单任务重计算、要隔离要限时    | worker pod（Argo Workflow），MLflow 打点 |
...
- **跨表流程**：execution_claim 防双写，前向恢复靠幂等重写；中间态由读侧过滤对下游隔离。
...
| 批次级     | execution_claim 互斥/fencing；Argo task retry；PG 最终业务态；Asset Check 对账 |
...
| 灵活流程编排（`manifest_op` + `pipeline_profile` + `processing_type`） | ✅ 进 | 操作语义、下游人工分支和处理策略三者正交 |
...
| Claim/Argo/业务终态分层                               | ✅ 进           | 见 3.3，`common/execution_claim.py` |
...
├── common/                   # config/db/iceberg/execution_claim/jobs/kafka 等共享库
├── engines/
│   ├── spark/                 # 入湖/freeze/导出 PySpark 脚本
│   ├── duckdb/                 # 打标签/分析查询脚本
│   ├── ray/                    # 预标（mock）/数据质检脚本
│   ├── training/               # 训练 run 侧控制面（claim→fan-out→归档，3.7）
│   └── worker/                 # worker pod 入口（ingest 解析 / 训练）+ staging 契约
├── tools/
│   ├── annotation_cli/        # 标注 CLI（模拟人在环）
│   └── datagen/                # 合成 MCAP + manifest 生成器（带质检故障注入开关）
├── schemas/                   # Iceberg 表 DDL / Postgres DDL / 建表脚本
├── docs/                      # 当前实现说明 + 顶部标明的历史归档
└── README.md
```
...
kubectl -n data port-forward svc/mlflow 5000:5000 &                   # MLflow UI（3.7）
kubectl -n data port-forward svc/argo-server 2746:2746 &              # Argo UI（worker 逐节点日志，https）
...
#    或走 gateway：POST /dataset-requests（支持 sample_ids 手工挑样本，3.7.2）

# 6b. 模型训练（3.7.5 有完整命令）：POST /train 发起 → training_kafka_sensor 拉起
#     model_training_job → MLflow UI（localhost:5000）看逐 epoch 曲线 →
#     GET /training-jobs/{job_id} 查终态 → promote API 流转版本
...
| `10-infra.yaml`  | 基础设施：Postgres、Kafka（KRaft 单节点）、MinIO（含建桶 Job）、Iceberg REST Catalog（catalog 元数据 JDBC 落 PG `iceberg_catalog` 库，防 pod 重启丢表）、OpenSearch（+Dashboards，tag 检索投影，见 3.5）、MLflow（复用 edp 镜像，backend=PG `mlflow` 库，见 3.7）  |
| `20-apps.yaml`   | 应用四件：dagster-code（gRPC code-location）、dagster-webserver、dagster-daemon、gateway，共用 `edp:dev` 镜像                        |
| `argo/namespace-install-v4.0.7.yaml` | Argo Workflows 官方 namespace 级安装（CRD + workflow-controller + argo-server），worker pod 监督引擎（3.6.3）      |
| `30-argo.yaml`   | Argo 的 EDP 叠加：artifact repository → MinIO（`archiveLogs: true`，pod 日志归档 `s3://lake/argo/`）、Workflow 完结 1 天自动清、dagster SA 提交 Workflow 的 RBAC |
...
| `apply.sh`       | 从源码目录动态生成 configmap（Postgres init SQL、dagster.yaml、workspace.yaml），再按序 apply 上面的 manifest（Argo 大 CRD 需 `--server-side`）                |
...
```bash
# 某次上传的业务终态 + 最近 Dagster run；task 重试/exit/log 去 Argo UI 查
kubectl -n data exec deploy/postgres -- psql -U edp -d platform \
  -c "SELECT upload_id, status, last_dagster_run_id, last_error_code, last_error
      FROM upload_session ORDER BY created_at DESC LIMIT 10;"
