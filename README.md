...
graph TD
    subgraph 触发层
        ...
        STUCK[platform_stuck_sensor: 各 JobKind 的 Saga 心跳看护]
    ...
    subgraph 存储层
        ...
        PG[(PostgreSQL: platform 库<br/>UploadSession/PlatformJob/AnnotationBatch/SagaLog/Alerts)]
...
| worker pod 监督 | Argo Workflows                              | namespace 级安装（v4.0.7），日志归档到 MinIO `s3://lake/argo/` | ingest/训练 worker pod 的扇出、并发、超时、终态记录、日志归档；run pod 只提交 Workflow CR 并轮询结果（3.6.3），业务重试语义仍由 saga + watchdog 决定 |
...
| 业务状态存储    | PostgreSQL（`platform` 库）                        | 独立 schema，与 Dagster 元数据库（`dagster` 库）、MLflow backend（`mlflow` 库）分开 | UploadSession、PlatformJob（通用状态机，3.1.2.1）、AnnotationBatch、SagaLog、Alerts、DatasetRequest |
...
| `platform_job`     | `job_id`               | **通用异步任务状态机**（`job_type` 区分类型，MVP 有 `training`），新任务类型不再各建一张表 | 网关写 ready；Kafka sensor 校验 ready；run 收尾改 done；watchdog 统一看护 |
...
- **协议（所有类型共享，一套实现）**：状态集 `ready → running → done / failed`；`(saga scope, business_id)` 一把互斥锁；`updated_at` 兼作悬置判定与 run_key 组成；`saga_log.error_code` + 退避决定自动重试；Kafka 触发 + watchdog 兜底双路径。看护逻辑（卡死重入队 / ready 悬置补触发 / failed 按码重试 / 次数耗尽转人工）只有 `common/jobs.py::watchdog_pass()` 一份实现。
- **定制点（每类型一条 `JobKind` 注册项）**：状态存到哪张表/哪些列（`upload` 绑定既有的 `upload_session`，字段名 `ingesting` 沿用；`training` 等新类型统一落 `platform_job`）、saga scope 怎么拼、补发哪条 Kafka 触发消息、退避/超时/内存档位读哪些 `runtime_config` 键。
...
| 字段            | 类型          | 说明                                          |
| ------------- | ----------- | ------------------------------------------- |
| `scope`       | TEXT        | **复合主键之一**，业务流程名：`ingest_append` / `ingest_correct` / `training` |
| `business_id` | TEXT        | **复合主键之一**，业务主键：`upload_id` 或 `job_id`      |
...
`runtime_config`（运行时参数，key/value 两列）：微批大小 `INGEST_BATCH_MAX`（默认 200）、背压上限 `INGEST_MAX_INFLIGHT_BATCHES`（默认 3）、保留天数 `RETENTION_DAYS`（默认 30），见 3.6.2/3.6.4；训练侧对应有 `TRAIN_MAX_INFLIGHT` / `TRAIN_WORKER_TIMEOUT_SECONDS` / `TRAIN_WORKER_MEMORY_TIERS` / `TRAIN_RETRY_BACKOFF_MINUTES` / `TRAIN_GATE_MIN_ACCURACY`（3.7）。放 PG 的原因与策略注册表相同：`UPDATE` 后 sensor 下一个 tick 即生效，不用重启任何组件。
...
sequenceDiagram
    ...
    DG->>PG: saga claim（scope=training）→ status=running
    ...
    DG->>PG: saga succeed → status=done（result 带指标摘要）
    rect rgb(255, 243, 205)
...
**批内执行形态：pod fan-out，由 Argo Workflows 监管（已实现）**。触发/背压设计只管"批"，与批内计算形态解耦；批内最重的活（下载 MCAP → 解析 → 清洗 → 切片 → 写 Lance）不在 run pod 里做，而是**每个 upload 一个 worker pod**。pod 级监督（扇出、并发、超时、终态、日志归档）全部交给 **Argo Workflows**（`common/argo_workflows.py`）——run pod 把一个批次渲染成一个 Workflow CR 提交，然后轮询等终态，不再自己管理任何 pod：
...
轮询 workflow status（每 tick 顺手刷 saga 心跳）    下载 MCAP → 流式解析/清洗 → 写 Lance
收 manifest.json（薄表行 + error_code 内联）◀──    业务失败也写清单（带状态码）后正常退出
  无清单 → 按 Argo 节点终态推断 OOM/超时/丢失      pod stdout 自动归档 s3://lake/argo/
每表每批一次 Iceberg commit（事务内分块追加）
succeed_many + session done；失败按码进重试层
```

- **worker 无状态**：只碰对象存储（staging / raw / quarantine）和 Lance 卷，**不连 PG、不碰 Iceberg catalog、不调 K8s API、不依赖任何平台库**。输入全部由 run pod 预先写进 `staging/{run_id}/{upload_id}/input.json`（session 快照、清洗策略入口、correct 模式的 episode 锚点、chunk 大小），worker 怎么死都不影响一致性——没有清单就等于没干过。
- **manifest 是数据契约，Argo 是监督与观测通道**：薄表行、厚表文件引用、`error_code` 的真相在 staging 的 `manifest.json` 里，worker 本地可脱离平台直接运行；每个 pod 的 stdout 由 Argo 归档到 `s3://lake/argo/{workflow}/{pod}`，pod 用后即删日志不丢，Argo UI（port-forward 2746）按 workflow → 节点逐个检索，workflow 带 `dagster-run-id` 标签可与 Dagster run 互查。
...
- **失败语义 = 状态码（`common/errors.py`）**：业务失败（数据的问题）→ worker 自报 `DATA_PARSE_ERROR`/`DATA_EMPTY` 等，写进 manifest 后正常退出（exit 0，Argo 视为节点成功，不会对数据错误傻重试）；pod 级失败 → 无清单，run 侧从 Argo 节点终态推断 `WORKER_OOM`/`WORKER_TIMEOUT`/`WORKER_LOST`（死掉的进程自报不了）；整批级 → `COMMIT_CONFLICT`/`PG_ERROR`。码落 `saga_log.error_code`，逐条 `fail_one` 隔离，同批其他照常。
...
- **超时两层**：worker pod 模板级 `activeDeadlineSeconds`（`INGEST_WORKER_TIMEOUT_SECONDS`，默认 600s）；workflow 级 deadline 兜底整批（含 pod 一直 Pending 调度不上的情况）。
- **并发**：Workflow 的 `spec.parallelism` 限制批内同时在跑的 worker 数（`INGEST_WORKER_MAX_PARALLEL`，默认 20），Argo controller 自动排队，不会一次打爆节点。worker 镜像与 run pod 同源（`edp-env` 的 `EDP_IMAGE`）；pod 完结即删（`podGC: OnPodCompletion`），Workflow CR 完结 1 天后自动清（`ttlStrategy`）；现场靠归档日志 + staging 清单 + `saga_log.error_code`；staging 残留由 retention job 按 mtime 清（`STAGING_RETENTION_DAYS`，默认 7 天，有意不在 run 结束时删——保留现场方便排查）。
...
5. ✅ pod fan-out（3.6.3）：`common/argo_workflows.py`（一个批次一个 Workflow CR：提交幂等、轮询等终态并刷 saga 心跳、按节点终态分类失败；替代早期自研的 `worker_pods.py`——PipesK8sClient + 线程池 + 监视线程那套 pod 监督代码已删，交给 Argo）、`engines/worker/staging.py`（input.json / manifest.json / 厚表 parquet 交接约定）、`engines/worker/ingest_parse.py`（worker 入口，流式解析 + 分块写 + 水位线切片，append/correct 两模式）。2026-07-17 切换到 Argo 后回归通过：ingest e2e / 训练成功（MLflow 打点齐全）/ DATA_EMPTY 失败不自动重试 / retry API 200+409 / pod 日志归档 MinIO 可检索。
...
模型也是 asset：它由确定的数据版本 + 确定的代码 + 确定的超参产出，理应进同一张血缘图、受同一套 saga/重试语义看护。这一节回答四个问题：MLflow 扮演什么角色、数据怎么变、Dagster 怎么改、状态机怎么改。
...
| 写入方      | 训练 worker **best-effort** 打点，挂了不影响训练   | run pod 单写者在终态时归档，走 saga 保证                      |
...
**一致性模型刻意不做双写事务**：worker 打点 MLflow 与 run pod 归档 Iceberg 是两条独立链路，前者尽力而为，后者是 saga 步骤。唯一的耦合点是归档时从 MLflow **抄**逐 epoch 曲线进 `ml_metric_epoch`——抄不到只影响这张曲线表（记 alert，档案主行不缺字段），不存在"两边都写一半"的中间态。模型权重文件的 SoT 在 MinIO `models/` 前缀（worker 直传 + sha256 指纹），MLflow artifact 里那份只是 UI 预览的副本。
...
run pod：saga claim（scope=training）
...
saga succeed → platform_job done（result 摘要）
```
...
| sensor | 新增 `training_kafka_sensor`（消费 `edp.jobs.requests`，校验 `platform_job.status=ready`，背压 `TRAIN_MAX_INFLIGHT`，一个 job 一个 RunRequest，run_key=`job_id+updated_at`）；`ingest_stuck_sensor` 泛化为 `platform_stuck_sensor`（见 3.7.4） |
...
失败语义完全复用 `common/errors.py`：数据集不存在/为空 → worker 自报 `DATA_EMPTY`（不自动重试）；训练 OOM → pod 终态推断 `WORKER_OOM`（自动重试 + `TRAIN_WORKER_MEMORY_TIERS` 升档）；MLflow 不可达不是错误（best-effort）；归档 commit 失败 → `COMMIT_CONFLICT`（自动重试，幂等重写）。超时用 `TRAIN_WORKER_TIMEOUT_SECONDS`（默认 1800s，训练比解析长）。

#### 3.7.4 状态机的泛化：一份协议，按类型注册

这是本次改动里对既有代码动得最深的一处。`upload_session` 的状态机语义（ready/running/终态 + saga 互斥 + 心跳看护 + 按码重试）已经过入湖链路验证，训练需要的是**同一套语义**，而不是一张新表加一个新 sensor 的复制品。做法（3.1.2.1 有协议定义）：

- **`common/jobs.py`**：`JobKind` 注册表描述每类任务的定制点——状态存哪张表哪些列、saga scope 怎么拼、补发什么 Kafka 触发、退避/上限读哪些配置键。`upload`（append/correct）注册为既有 `upload_session` 表的**适配绑定**（表结构一字不动，`ingesting` 这个历史状态名也保留）；`training` 及未来类型统一绑定 `platform_job` 表。
- **`watchdog_pass(kind)`**：把原 `ingest_stuck_sensor` 的三类修复（卡死重入队 / ready 悬置补触发 / failed 按码退避重试，attempt 耗尽转 failed+alert）抽成对 `JobKind` 泛化的一份实现；`platform_stuck_sensor` 每个 tick 对所有注册类型各跑一遍。修复动作仍然只是"重置状态 + 补发 Kafka 消息"，真正的互斥由引擎侧 saga claim 的 CAS 保证——这层不变。
- **网关的 retry 也是协议的一部分**：`POST /training-jobs/{id}/retry` 与 `POST /sessions/{id}/retry` 行为同构（仅 failed→ready + 补发触发），实现共用一段。

判断"要不要用这套协议"的标准：任务是否异步、是否可能失败重试、是否需要防并发双写。满足就注册 `JobKind`；一次性同步操作（如 promote）不需要状态机，直接做。

**状态转换 ↔ 代码 ↔ saga 对应表**（以 training 为例，upload 同构、仅表名/scope 不同）：

| 状态转换 | 谁触发 | 代码位置 | saga_log 侧动作 |
| --- | --- | --- | --- |
| （无）→ `ready` | 网关 `POST /train` | `common/jobs.py::create_job` | 无（saga 行尚不存在） |
| `ready` → `running` | run pod 抢到锁后 | `engines/training/run_job.py::run_training` | `claim()`：CAS 置 RUNNING，`attempt` +1 |
| `running`（保持） | worker 波次期间心跳 | `launch_wave(heartbeat=saga.advance)` | `advance(step)`：推进步骤 + 刷心跳 + fencing |
| `running` → `done` | run pod 归档完成 | `run_job.py::_execute` 末尾 | `succeed()`：置 SUCCEEDED |
| `running` → `failed` | run pod（worker 自报码 / pod 终态推断 / run 侧异常） | `run_job.py::_fail` | `fail(error_code)`：置 FAILED + 落码 + alert |
| `running` → `ready`（卡死重入队） | watchdog：心跳超时且 `attempt` 未达上限 | `common/jobs.py::watchdog_pass` 第 ① 段 | 不动 saga 行——新 run `claim()` 时自然接管、`attempt` +1 |
| `running` → `failed`（耗尽） | watchdog：心跳超时且 `attempt` 达上限 | `watchdog_pass` 第 ① 段 else 分支 | CAS 置 FAILED（`STUCK_EXHAUSTED`）+ alert |
| `ready`（补触发） | watchdog：悬置超 `SAGA_TAKEOVER_MINUTES` | `watchdog_pass` 第 ② 段 | 不涉及（刷新 `updated_at` → 新 run_key） |
| `failed` → `ready`（自动重试） | watchdog：错误码 RETRYABLE 且退避到期 | `watchdog_pass` 第 ③ 段 | 保留 FAILED 行；下次 `claim()` 接管时 `attempt` +1 |
| `failed` → `ready`（人工重试） | 网关 `POST /training-jobs/{id}/retry` | `common/jobs.py::manual_retry` | 同上（仅 failed 可重试，否则 409） |
| `done`（终态） | UI Re-execute / 重复消息 | `run_job.py` 入口按 `status<>'done'` 过滤 | 不再触碰 |

两条不变式贯穿整表：**互斥永远由 `claim()` 的 CAS 保证**（watchdog/retry 只是"重置状态 + 补发消息"，就算误判也不会双写）；**归档以 `job_id` 为幂等键**（upsert/replace_where），任何一次重跑覆盖同一份档案而不是追加副本。
...
7. ✅ 端到端回归（2026-07-17 minikube，VM 内存 6G→10G，Spark freeze 的 JVM 固定开销会挤死 6G 环境）：上传 3 mcap → 入湖 31 样本 → 冻结 `v20260717085719`（28 样本）→ `POST /train` → sensor 拉起 run → worker 训练 + MLflow 打点（run/参数/6 点 epoch 曲线/registry v1 全部可查）→ Iceberg 四表归档 → `platform_job done`；失败路（`v-not-exist` → `DATA_EMPTY`，NOT_RETRYABLE 不自动重试，saga attempt 正确累加）；retry API（failed→200 重置 ready，done→409 拒绝）；promote API（MLflow alias=production + `ml_promotion_event` 落账本）。
...
| 故障自愈       | `platform_stuck_sensor` 对每个注册的 `JobKind`（upload/training）按 saga 心跳找卡死任务，重置 + 补发触发消息（见 3.3.3 / 3.7.4） | 入湖/训练崩溃恢复          |
...
├── common/                   # config/db/iceberg/saga/jobs（通用状态机）/kafka_ledger 等共享库
├── engines/
