...
本仓库是我博客《云平台及遥操设计》一文第四节"数据平台"的 MVP 实现。原文设计的是对标 PB 级数据量、多团队协作的完整生产架构；这个仓库实际上是想做一个最小可用版本出来，验证可行性。
...
在我个人看来，数据平台根本要解决的是，工作流编排+数据血缘治理。数据平台的CRUD部分比较简单，这个实际上归属于数据血缘治理。
