...
`entity_tag`：复合主键 `(target_type, target_id, tag_key)`，同一对象同一 key 仅有一个值，重新打标即覆盖。`tag_key/tag_value` 采用全 string 的 KV 泛化设计：key 不预定义 schema，作为圈选数据的主要维度；代价是数值比较需 cast（后续可接 OpenSearch 补检索能力）。由 `engines/duckdb/entity_tag.py` 按规则策略（3.1.2.2）增量打标。
...
sequenceDiagram
    ...
    K-->>DG: ingest_kafka_sensor 消费（offset 存 sensor cursor）
    DG->>PG: 校验 status = ready（重放消息在此廉价跳过）
    DG->>RUN: RunRequest（run_key 去重，op=append/correct 路由到不同 job）

    RUN->>PG: saga.claim() CAS 抢占（抢不到立即退出，防双写）
    RUN->>ICE: PARSE→INDEX→BRONZE→SILVER→SAMPLES（幂等写，见 3.3.2）
...
1. **Kafka offset 存在 sensor cursor 里**，与 `RunRequest` 的提交一起被 Dagster 持久化，不用 Kafka 自身的 group commit——避免"offset 提交了、run 没发出去"的缝隙；消息重放时 sensor 先查 PG，只有 `status=ready` 才发 run（廉价跳过）。
2. `**run_key = op-upload_id-updated_at`**：Dagster 对同一个 run_key 只创建一次 run。Kafka sensor 和 T+1 兜底 schedule（`orchestration/schedules.py`，防 Kafka 消息丢失/发送失败）共用这个生成规则，两条触发路径互相去重；`updated_at` 参与 run_key 是为了让"stuck 重试重置回 ready"能产生新 run_key（否则纯 `op-upload_id` 消费一次就永远无法重试）。
3. **引擎侧 `saga.claim()` CAS 互斥**（见 3.3）：就算前两层都被绕过，同一个 upload 也只有一个写者。
...
| 触发需求       | Dagster 机制                                                                                  | 用在哪               |
| ---------- | ------------------------------------------------------------------------------------------- | ----------------- |
| 近实时事件触发    | `ingest_kafka_sensor` 消费 `edp.ingest.requests`（offset 存 sensor cursor，与 RunRequest 提交原子持久化） | 上传入湖（见 3.2.1）     |
| 定时 T+1 兜底  | `ScheduleDefinition` 轮询 PG `status=ready`，与 Kafka sensor 共用 run_key 规则互相去重                  | 入湖（防 Kafka 消息丢失）  |
...
| `10-infra.yaml`  | 基础设施四件：Postgres、Kafka（KRaft 单节点）、MinIO（含建桶 Job）、Iceberg REST Catalog                                                  |
