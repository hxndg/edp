...
2. run 从 PG 一次 JOIN 冻结 processing definition 与不可变执行 Profile，再批量 acquire claim，写最后 run/profile。
...
和 `last_execution_profile_id`。Workflow annotation 记录 processing type、profile 与 image。
